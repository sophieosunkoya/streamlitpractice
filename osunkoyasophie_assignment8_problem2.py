...
for i in range(0, len(secret)):
    ...
    if (type(secret[i]) == str):
        comandToRun = secret[i]
        
    if (type(secret[i]) == int and counter == 2):
        xAxis = secret[i]

    if (type(secret[i]) == int and counter == 3):
        yAxis = secret[i]

    if (comandToRun != "" and xAxis != 0 and yAxis != 0):
        print(comandToRun, xAxis, yAxis)
        if (comandToRun == 'S'):
            turtle.penup()
